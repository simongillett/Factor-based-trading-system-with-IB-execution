...
def main():
    ...
    companies = [c for c in companies if c['symbol'] not in existing]
    print(f"Skipping {len(existing)} already fetched, {len(companies)} remaining")
    ...
    print(f"Completed! {result[0]} total price records")
    conn.close()
